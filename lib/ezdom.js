...
(function ($) {
  ...
  EZDOM.updateElement = function (left, right) {
    ...
    for (var k = 0; k < right[0].childNodes.length; k++)
      if (listContains(left[0].childNodes, right[0].childNodes[k], this.tagsMatch, this)) {
        var filterCallback = function (element) {
          if (element[0]) return EZDOM.tagsMatch(right[0].childNodes[k], element[0]);
          else return EZDOM.tagsMatch(right[0].childNodes[k], element);
        };
    ...
    for (var p = 0; p < left[0].childNodes.length; p++)
      this.updateElement($(left[0].childNodes[p]), $(right[0].childNodes[p]).clone(true, true));
  };
})(jQuery);

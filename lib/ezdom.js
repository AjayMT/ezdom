...
(function () {
  ...
  EZDOM.updateElement = function (left, right) {
    ...
    if (right.childNodes.length === 0 || ! this.tagsMatch(left, right)) {
      left.outerHTML = right.outerHTML;
    ...
    for (var j = 0; j < left.childNodes.length; j++)
      if (! listContains(right.childNodes, left.childNodes[j], this.tagsMatch, this))
        removed.push(left.childNodes[j]);

    for (var k = 0; k < right.childNodes.length; k++)
      if (listContains(left.childNodes, right.childNodes[k], this.tagsMatch, this)) {
    ...
    for (var m = 0; m < removed.length; m++)
      removed[m].parentNode.removeChild(removed[m]);
    ...
    for (var p = 0; p < left.childNodes.length; p++)
